...
- `packages/web/server/lib/notifications/apns-runtime.js`: native iOS APNs device-token persistence + delivery. Two modes: **relay** (default — POST tokens + generic text to the central Cloudflare relay `https://api.openchamber.dev/v1/push/send`, which holds the single project APNs key) and **direct** (fallback — sign ES256 JWT with Node crypto + HTTP/2, when `OPENCHAMBER_PUSH_RELAY_DISABLED=true`). Shares the push runtime's UI visibility gate via `isAnyUiVisible`. Mobile push carries only generic, model-based text — see `APNS.md`. Local Notifications (`packages/ui/src/apps/nativeNotifications.ts`) cover the foreground/brief-background window.
